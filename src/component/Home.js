...
const Home = () => {
  ...
  //   const [ data , setData] = useState({
    ...
    function myEncrypt( input ){
      let output = '';
    //  alert(typeof input)
      input.split('').forEach( ( v ,k) => output+= hack.indexOf(v)===-1?'x'+v:(hack.indexOf(v)+1 < 10?'0'+ (hack.indexOf(v)+1) :hack.indexOf(v)+1 ))
      return output
  }
     ...
     useEffect(() => {
      ...
      const storeDataInFirebase =()=>{
        db.collection(context.user.uid).doc("data").set( data )
       .then(function() {
  ...
  function myDecrypt(input){
    let output =''
    for(let i=0;i<input.length;i+=2) output += input[i]==='x'?input[i+1]:(input[i]!=='0'?hack[Number(input[i]+input[i+1])-1]:hack[Number(input[i+1])-1])
    return output
}
  ...
  return (
    <div className="home">
     ...
     <Button className='mt-2' color="primary" onClick={toggle}>
      ...
      <Modal isOpen={modal} toggle={toggle}  className='cardModal'>
        <ModalHeader toggle={toggle}>Account Name</ModalHeader>
        <ModalBody >
        <input type='text' id='accInput' className='border-0 p-2'  placeholder='Enter your input' ></input>
        </ModalBody>
        <ModalFooter>
       ...
       <div className="cards">
        {accounts.map((acc, key) => (
                 <Card  storeDataInFirebase = {storeDataInFirebase}
                  data={data} setData={ setData} cardToggle = { cardToggle  }
                    title={acc} key={key}  accounts={ accounts } setAccounts={ setAccounts } />
        ))}

        <Modal isOpen={cardModal} toggle={cardToggle} className='view'  >
      ...
      <AddTbody />
        ...
        <ModalFooter>
          <Button color="primary" onClick={ addInList}>Add</Button>{' '}
          <Button color="secondary" onClick={cardToggle}>Cancel</Button>
        </ModalFooter>
      </Modal>
      </div> 
      <ToastContainer  />
